	...
	def getPathTrace(self):
		...
		for segment in self.segments:
			if segment.orientation == "h":
				isReversed = segment.start.x < segment.end.x
				inc = 1 if isReversed else -1
				trace.extend([Point(x, segment.start.y) for x in range(segment.start.x + inc, segment.end.x + inc, inc)])
			else:
				isReversed = segment.start.y < segment.end.y
				inc = 1 if isReversed else -1
				trace.extend([Point(segment.start.x, y) for y in range(segment.start.y + inc, segment.end.y + inc, inc)])
...
def getStepCountForIntersection(paths, intersection):
	totalStepCount = 0
	for path in paths:
		stepCount = 0
		for point in path:
...
wires = readWires()
paths = [wire.getPathTrace() for wire in wires]
intersections = getIntersectionPointsOfWires(wires)

bestStepCount = getStepCountForIntersection(paths, intersections[0])
for idx in range(1, len(intersections)):
	bestStepCount = min(bestStepCount, getStepCountForIntersection(paths, intersections[idx]))
